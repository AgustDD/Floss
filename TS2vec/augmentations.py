...
def one_hot_encoding(X):
    """独热编码，将标签转换为整数类型，根据类别数量创建独热编码数组，最后将整数标签转换为对应的独热编码"""
    X = [int(x) for x in X]
    n_values = np.max(X) + 1
    b = np.eye(n_values)[X]
    return b
...
def DataTransform(sample, config):
    ...
    """弱增强只应用了缩放scaling，强增强在缩放前还进行置换和抖动"""
...
#     return weak_aug, strong_aug
def DataTransform_TD(sample, config):
    """时间域转换函数 Simplely use the jittering（抖动） augmentation. Feel free to add more autmentations you want,
...
def DataTransform_TD_bank(sample, config):
    """时间域数据转换函数，包括四种增强方式，随机选择一种为正样本，函数内部随机生成数字进行选择和加权，最终返回加权后的样本"""
...
def DataTransform_FD(sample, config):
    """Weak and strong augmentations in Frequency domain 频域数据转换函数，包括频率去除remove_frequency和频率增加add_frequency两种增强方式，函数将这两种增强方式的结果相加"""
    device = config.device
    aug_1 = remove_frequency(sample, pertub_ratio=0.1)
    aug_2 = add_frequency(device,sample, pertub_ratio=0.1)
    aug_F = aug_1 + aug_2
    return aug_F

def remove_frequency(x, pertub_ratio=0.0):
    """去除频率：随机生成与输入形状相同的掩码矩阵，一部分作为true（保留），一部分做false（删除）"""
    mask = torch.cuda.FloatTensor(x.shape).uniform_() > pertub_ratio # maskout_ratio are False
    mask = mask.to(x.device)
    return x*mask

def add_frequency(device, x, pertub_ratio=0.0 ):
...
def jitter(x, sigma=0.8):
    ...
    m = x.cpu().numpy()
    return m + np.random.normal(loc=0., scale=sigma, size=x.shape)
